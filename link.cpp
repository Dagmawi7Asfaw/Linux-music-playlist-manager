...
// inline in link.h - these handle case-insensitive string operations and file path cleaning
...
void Stack::push(node* item) {
  stackNode* newStackNode = new stackNode(item, topPtr);  // Create new node with item and link to current top
...
void LinkedList::add_beg(const std::string& song, const std::string& artist) {
  ...
  if (head == nullptr) {  // Case 1: List is currently empty
    ...
    while (last->next != head) {       // Traverse until we find node pointing to head
      last = last->next;
    }
    newNode->next = head;  // New node points to the old head
    head = newNode;        // Update head to point to the new node
    last->next = head;     // Update last node to point to new head (maintaining circular structure)
  }
...
void LinkedList::add_end(const std::string& song, const std::string& artist) {
  ...
  if (head == nullptr) {  // Case 1: List is empty
    ...
    while (temp->next != head) {       // Traverse until we find node pointing to head
      temp = temp->next;
    }
...
void LinkedList::display() const {
  ...
  std::cout << "\t\t\t------------------------------------" << std::endl;
  if (head == nullptr) {
    std::cout << std::endl << "\t\t\t(List is empty)" << std::endl;  // Message for empty list
  } else {
    node* temp = head;                     // Start at head node
    int count = 1;                         // Track position for display
    do {
      // Get clean name using helper from link.h
      std::string cleanName = getCleanSongName(temp->song);  // Extract filename from path
      ...
      std::cout << "\t\t" << std::left << std::setw(3) << count++ << ". "
                ...
                << " -- " << temp->artist.substr(0, 20)  // Truncate artist name if too long
                << std::endl;
      temp = temp->next;                   // Move to next node
...
void LinkedList::clear() {
  ...
  head->next = nullptr;        // IMPORTANT: Break the circle FIRST to avoid infinite loop
...
void LinkedList::search(const std::string& searchTerm) const {
  ...
  lowerSearchTerm.reserve(searchTerm.size());  // Pre-allocate memory for efficiency
  ...
  do {
    ...
    if (lowerListSong.find(lowerSearchTerm) != std::string::npos) {  // String found
...
void LinkedList::sortBySong() {
  ...
  do {
    ...
    // After one full pass, the largest element is at the 'end' relative to the start
...
void LinkedList::sortByArtist() {
  if (head == nullptr || head->next == head) return;  // Already sorted if 0 or 1 element
  ...
  do {
    ...
    for (int i = 0; i < len - 1; ++i) {
      ...
      std::transform(current->artist.begin(), current->artist.end(),
                     std::back_inserter(lowerCurrentArtist), ::tolower);  // Current artist to lowercase
      std::transform(nextNode->artist.begin(), nextNode->artist.end(),
                     std::back_inserter(lowerNextArtist), ::tolower);  // Next artist to lowercase
...
bool LinkedList::loadFromFile(const std::string& filename) {
  try {
    ...
    if (playlistJson.contains("listName") &&
      ...
      listName = playlistJson["listName"].get<std::string>();  // Set playlist name from JSON
    } else {
    ...
    if (playlistJson.contains("songs") && playlistJson["songs"].is_array()) {
      // Get the songs array
      const json& songsArray = playlistJson["songs"];  // Reference to the songs array

      // Process each song in the array
      for (const auto& songObj : songsArray) {  // Iterate through each song object
        if (songObj.contains("song") && songObj.contains("artist") &&
            songObj["song"].is_string() && songObj["artist"].is_string()) {
          std::string songPath = songObj["song"].get<std::string>();      // Get song path
          std::string artistName = songObj["artist"].get<std::string>();  // Get artist name
    ...
    if (playlistJson.contains("length") &&
      ...
      int expectedLen = playlistJson["length"].get<int>();  // Get expected length
      if (len != expectedLen) {
